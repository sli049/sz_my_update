...
size_t dataLength, double realPrecision, size_t *outSize, float valueRangeSize, float medianValue_f, int phase)
{		
	char compressionType = 0;	
	TightDataPointStorageF* tdps = NULL;	
//#if 0
	...
	{
		int timestep = sz_tsc->currentStep;
		if(timestep % confparams_cpr->snapshotCmprStep != 0 && phase == 1)
		{
			tdps = SZ_compress_float_1D_MDQ_ts(oriData, dataLength, multisteps, realPrecision, valueRangeSize, medianValue_f);
			compressionType = 1; //time-series based compression 
		}
...
int errBoundMode, double absErr_Bound, double relBoundRatio, double pwRelBoundRatio)
{
	...
	int status = SZ_SCES;
	size_t dataLength = computeDataLength(r5,r4,r3,r2,r1);
	//if (partial) dataLength = sz_tsc->intersect_size;
		...
		if (r2==0)
		{
			if(confparams_cpr->errorBoundMode>=PW_REL)
			{
				...
				if(confparams_cpr->szMode == SZ_TEMPORAL_COMPRESSION)
					...
					//multisteps->compressionType = SZ_compress_args_float_NoCkRngeNoGzip_1D(&tmpByteData, oriData, dataLength, realPrecision, &tmpOutSize, valueRangeSize, medianValue);
				else
#endif				
...
int errBoundMode, double absErr_Bound, double relBoundRatio, double pwRelBoundRatio, int partial, int phase)
{
	...
	if (partial && phase == 2) {
		...
		oriData = oriData + sz_tsc->intersect_size;
	}
		...
		if (r2==0)
		{
			if(confparams_cpr->errorBoundMode>=PW_REL)
			{
				...
				if(confparams_cpr->szMode == SZ_TEMPORAL_COMPRESSION){
					...
					multisteps->compressionType = SZ_compress_args_float_NoCkRngeNoGzip_1D_ps(&tmpByteData, oriData, dataLength, realPrecision, &tmpOutSize, valueRangeSize, medianValue, phase);
				}
				else
#endif				
					SZ_compress_args_float_NoCkRngeNoGzip_1D(&tmpByteData, oriData, r1, realPrecision, &tmpOutSize, valueRangeSize, medianValue);
...
unsigned char * SZ_compress_float_2D_MDQ_nonblocked_with_blocked_regression(float *oriData, size_t r1, size_t r2, double realPrecision, size_t * comp_size){
	...
	int reqLength_a = generateLossyCoefficients_float(reg_params, precision_a, num_blocks, &reqBytesLength_a, &resiBitsLength_a, &medianValue_a, reg_pred_params);
	int reqBytesLength_b = 0, resiBitsLength_b = 0; 
	int reqLength_b = generateLossyCoefficients_float(reg_params+params_offset_b, precision_b, num_blocks, &reqBytesLength_b, &resiBitsLength_b, &medianValue_b, reg_pred_params+params_offset_b);
	int reqBytesLength_c = 0, resiBitsLength_c = 0; 
	int reqLength_c = generateLossyCoefficients_float(reg_params+params_offset_c, precision_c, num_blocks, &reqBytesLength_c, &resiBitsLength_c, &medianValue_c, reg_pred_params+params_offset_c);			
...
unsigned char * SZ_compress_float_3D_MDQ_nonblocked_with_blocked_regression(float *oriData, size_t r1, size_t r2, size_t r3, double realPrecision, size_t * comp_size){
	...
	int reqLength_d = generateLossyCoefficients_float(reg_params+params_offset_d, precision_d, num_blocks, &reqBytesLength_d, &resiBitsLength_d, &medianValue_d, reg_pred_params+params_offset_d);	 			
