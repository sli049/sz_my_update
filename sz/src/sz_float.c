...
#ifdef HAVE_TIMECMPR	
	float* decData = NULL;
	if(confparams_cpr->szMode == SZ_TEMPORAL_COMPRESSION && phase == 1)
		decData = (float*)(multisteps->hist_data);
#endif	
...
//#if 0
	...
	if(confparams_cpr->szMode == SZ_TEMPORAL_COMPRESSION)
	{
		...
		if(phase == 2)
		{	
			tdps = SZ_compress_float_1D_MDQ_ps(oriData, dataLength, realPrecision, valueRangeSize, medianValue_f, phase);
			//compressionType = 0; //snapshot-based compression
			//multisteps->lastSnapshotStep = timestep;
		}		
	}
